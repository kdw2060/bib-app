    ...
    var Feeds = function (container, params) {
        ...
        var defaults = {
            ...
            formatDate: function (date) {
                date = new Date(date);
                var months = ('Jan Feb Mrt Apr Mei Jun Jul Aug Sep Oct Nov Dec').split(' ');
                return date.getDate(date) + ' ' + months[date.getMonth(date)] + ', ' + date.getFullYear();
            }
        };
        params = params || {};
            ...
            '<div class="view navbar-fixed">' +
                ...
                '<div class="pages">' +
                    '<div class="page feeds-page" data-page="feeds-page-{{index}}">' +
                        '<div class="page-content" id="fullarticle">' +
                            '<div class="content-block"><div class="content-block-inner">{{description}}</div></div>' +
                            '<div class="content-block">' +
                                '<a href="{{link}}" class="external" target="_blank">Lees het volledige bericht (opent in browser)</a><br>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            ...
            $.get(f.params.url, function (response) {
                ...
                items.each(function (index, el) {
                    ...
                    var itemData = {
                        ...
                        pubDate: item.children('date').text(),
                        formattedDate: f.params.formatDate(item.children('date').text()),
